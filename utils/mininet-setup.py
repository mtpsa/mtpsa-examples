...
class ExerciseRunner:
    """
        Attributes:
            ...
            user        : string // json of the compiled p4 example
    ...
    def __init__(self, topo_file, log_dir, pcap_dir,
                 switch_json, user, bmv2_exe='simple_switch', quiet=False):
        """ Initialise attributes and read the topology json.
            ...
            Arguments:
                ...
                user        : string  // Path to a compiled p4 json for bmv2
    ...
    def program_switch_p4runtime(self, sw_name, sw_dict):
        ...
        with open(runtime_json, 'r') as sw_conf_file:
            outfile = os.path.join(self.log_dir, sw_name + '-p4runtime-requests.txt')
    ...
    def program_switch_cli(self, sw_name, sw_dict):
        ...
        with open(cli_input_commands, 'r') as fin:
            ...
            with open(cli_outfile, 'w') as fout:
                subprocess.Popen(
                    [self.bmv2_exe + '_CLI', '--thrift-port', str(sw_obj.thrift_port)],
...
def get_args():
    ...
    parser.add_argument('-b', '--behavioral-exe', help='Path to the P4 behavioral binary', type=str, required=False, default='simple_switch')
    parser.add_argument('-j', '--switch-json', help='Path to a compiled P4 json for bmv2', type=str, required=False)

    parser.add_argument('-u', '--user', action='append', nargs=1, metavar=('user_config'),
                        help='<user-id>@<json-config> Path to a compiled P4 json', required=False)
