...
if __name__ == '__main__':
    ...
    dec_ip = decodeIP(enc_ip)
    assert(ip == dec_ip)

    ip = "fe80::a00:ff:fe00:200"
    enc_ip = encodeIP(ip)
    assert(enc_ip == b'\xfe\x80\x00\x00\x00\x00\x00\x00\n\x00\x00\xff\xfe\x00\x02\x00')
    dec_ip = decodeIP(enc_ip)
    assert(ip == dec_ip)
    ...
    try:
        ...
        assert(str(e) == "Number, 256, does not fit in 8 bits")
