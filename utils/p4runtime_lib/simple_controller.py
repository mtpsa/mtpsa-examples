...
def program_switch(addr, device_id, sw_conf_file, workdir, proto_dump_fpath):
    ...
    p4info_helper = P4InfoHelper(p4info_fpath)
    ...
    if target == "bmv2":
        sw = Bmv2SwitchConnection(address=addr, device_id=device_id,
    ...
    try:
        ...
        if target == "bmv2":
            ...
            sw.SetForwardingPipelineConfig(p4info=p4info_helper.p4info, bmv2_json_file_path=bmv2_json_fpath)
        else:
